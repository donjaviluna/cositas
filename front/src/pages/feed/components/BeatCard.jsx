    ...
    return (
        <div className='w-48 h-64 rounded-lg flex flex-col justify-center items-center gap-2'>
            <div className='h-4/5 rounded-xl overflow-hidden' onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
                <div className='w-full h-full relative cursor-pointer'>
                    ...
                    <div className={`w-full h-full flex justify-center items-center absolute inset-0 opacity-0 hover:opacity-100 transition duration-300`}>
                        <Icon icon="ic:baseline-play-circle" className='w-12 h-12 text-zinc-600 hover:text-zinc-900 transition duration-300' onClick={()=>setActualSong(beat)}/> 
                    </div>
                        }
                </div>
            ...
            <div className='h-1/5 w-full flex flex-col justify-center items-center'>
                <div className='h-1/2 w-full flex justify-start items-start px-2 gap-2'>
                ...
                <div className='w-full h-1/2 px-2 flex justify-start items-start'>
                    <span className='font-inter text-white font-semibold cursor-pointer hover:underline truncate'>{beat.name}</span>
                </div>
